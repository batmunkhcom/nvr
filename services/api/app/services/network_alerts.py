...
class NetworkAlertService:
    ...
    async def evaluate(self, camera_id: UUID, metrics: dict[str, Any], engine=None):
        ...
        checks = [
            ('bandwidth_low', metrics.get('outbound_mbps'), float(config['bandwidth_crit_mbps']), 'critical', 'Mbps'),
            ('bandwidth_warn', metrics.get('outbound_mbps'), float(config['bandwidth_warn_mbps']), 'warning', 'Mbps'),
            ('latency_high', metrics.get('rtt_ms'), float(config['latency_crit_ms']), 'critical', 'ms'),
            ('latency_warn', metrics.get('rtt_ms'), float(config['latency_warn_ms']), 'warning', 'ms'),
            ('packet_loss_high', metrics.get('packet_loss_pct'), float(config['packet_loss_crit_pct']), 'critical', '%'),
            ('packet_loss_warn', metrics.get('packet_loss_pct'), float(config['packet_loss_warn_pct']), 'warning', '%'),
        ]
        ...
        for alert_type, current_value, threshold, severity, unit in checks:
            ...
            alert = await self._create_alert(
                camera_id, alert_type, severity, msg,
    ...
    def _build_message(self, alert_type: str, current_value: float, threshold: float, unit: str) -> str:
    ...
    async def _create_alert(self, camera_id: UUID, alert_type: str, severity: str, message: str, metadata: dict, engine) -> dict:
