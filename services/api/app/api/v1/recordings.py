...
async def _generate_thumbnail(segment_path: str, thumb_path: str) -> bool:
    """Best-effort FFmpeg thumbnail extraction (mirrors recording-engine logic)."""
    import asyncio
    ...
    for seek in ("2", "0"):
        try:
            ...
            logger.warning(
                "recording_thumbnail_ffmpeg_failed",
            ...
            logger.warning(
                "recording_thumbnail_generation_failed",
                segment=segment_path,
                seek=seek,
                error=str(exc),
            )
...
async def thumbnail_recording(
    ...
    If the sidecar thumbnail is missing, generate it on demand from the segment.
    """
    recording = await get_recording(recording_id, db)
    segment_path = recording.file_path
    if not segment_path or not os.path.exists(segment_path):
        raise HTTPException(status_code=404, detail="Recording file not found on disk")

    thumb = os.path.splitext(segment_path)[0] + ".jpg"
    if not os.path.exists(thumb):
        logger.info("recording_thumbnail_on_demand", recording_id=str(recording_id), path=segment_path)
        if not await _generate_thumbnail(segment_path, thumb):
